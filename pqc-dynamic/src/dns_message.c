...
bool is_dns_message(unsigned char *in, size_t in_len) {
  ...
  for (uint16_t i = 0; i < qdcount; i++) {
    ...
    Question *q;
    if (question_from_bytes(cur_pos, len_left, &bytes_processed, &q) != 0) {
      return false;
    }
    question_destroy(&q);
    cur_pos += bytes_processed;
  ...
  for (uint16_t i = 0; i < ancount; i++) {
    ...
    size_t bytes_processed = 0;
    if (is_rrfrag(cur_pos)) {
      RRFrag *rrf;
      if (rrfrag_from_bytes(cur_pos, len_left, &bytes_processed, query, &rrf) !=
          0) {
        rrfrag_destroy(&rrf);
        return false;
      }
      ...
      ResourceRecord *rr;
      if (resource_record_from_bytes(cur_pos, len_left, &bytes_processed,
                                     &rr) != 0) {
