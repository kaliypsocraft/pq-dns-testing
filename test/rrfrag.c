...
int rrfrag_from_bytes(unsigned char *in, size_t in_len, size_t *bytes_processed,
  ...
  RRFrag *res = malloc(sizeof(RRFrag));
  if (res == NULL) {
    printf("Error rrfrag_from_bytes\n");
    return -1;
  }
  ...
  *bytes_processed = 1;
  ...
  // get type, and ensure it's an RRFrag. If not, then back out with special
  // error
...
bool is_rrfrag_equal(RRFrag *lhs, RRFrag *rhs) {
  ...
  if (lhs->fragdata != NULL && rhs->fragdata == NULL)
    return false;
  if (lhs->fragdata == NULL && rhs->fragdata != NULL)
    return false;
  bool fragdataCheck;
  ...
  return (nameCheck && typeCheck && fragsizeCheck && curidxCheck &&
          rrsizeCheck && rridCheck && fragdataCheck);
}
...
char *rrfrag_to_string(RRFrag *rrfrag) {
  ...
  size_t str_len =
      snprintf(NULL, 0,
               "RRFrag:\n\tNAME: %s\n\tTYPE: %hu\n\tFRAGSIZE: %hu\n\tCURIDX: "
               "%u\n\tRRSIZE: %u\n\tRRID: %hu\n\tFRAGDATA: *OMITTED*\n",
               rrfrag->name, rrfrag->type, rrfrag->fragsize, rrfrag->curidx,
               rrfrag->rrsize, rrfrag->rrid) +
      1;
  res = malloc((sizeof(char) * str_len));
  wanted_to_write =
      snprintf(res, str_len,
               "RRFrag:\n\tNAME: %s\n\tTYPE: %hu\n\tFRAGSIZE: %hu\n\tCURIDX: "
               "%u\n\tRRSIZE: %u\n\tRRID: %hu\n\tFRAGDATA: *OMITTED*\n",
               rrfrag->name, rrfrag->type, rrfrag->fragsize, rrfrag->curidx,
               rrfrag->rrsize, rrfrag->rrid);
