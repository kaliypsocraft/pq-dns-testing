...
int question_from_bytes(unsigned char *in, size_t in_len,
  ...
  name_byte_len += 1;
  rc = dns_name_from_bytes(in, &qname, &name_len, bytes_processed,
                           name_byte_len);
  if (rc != 0) {
    return -1;
  }
  ...
  rc = question_create(&res, qname, qtype, qclass);
  if (rc == 0) {
    *out = res;
...
int question_to_bytes(Question *in, unsigned char **out, size_t *out_len) {
  ...
  size_t qname_len;
  rc = dns_name_to_bytes(in->qname, strlen(in->qname), &qname, &qname_len);
  if (rc != 0) {
    *out = NULL;
    return -1;
  }
...
char *question_to_string(Question *q) {
  ...
  wanted_to_write = snprintf(
      res, str_len, "Question:\n\tQNAME: %s\n\tQTYPE: %hu\n\tQCLASS: %hu\n",
      q->qname, q->qtype, q->qclass);
