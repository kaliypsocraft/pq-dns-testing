...
int resource_record_from_bytes(unsigned char *in, size_t in_len,
  ...
  name_byte_len += 1;
  rc = dns_name_from_bytes(cur_pos, &name, &name_len, bytes_processed,
                           name_byte_len);
  if (rc != 0) {
...
bool is_resource_record_equal(ResourceRecord *lhs, ResourceRecord *rhs) {
  ...
  if (lhs->name_byte_len != rhs->name_byte_len)
    return false;
  ...
  return (nameCheck && typeCheck && classCheck && ttlCheck && rdsizeCheck &&
          rdataCheck);
}
